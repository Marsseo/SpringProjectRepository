function rgbled(command, red, green, blue) {
	...
	console.log(json.command,json.red, json.green, json.blue);
	$.ajax({
		url : "http://" + location.host + "/SpringWebProject/rgbled",
		data : json,
		method : "post",
...
/*YUI().use('slider','color',function(Y) {
	...
	function updateColors() {
		 r = rSlider.get('value'), g = gSlider.get('value'), b = bSlider
				.get('value'), rgbStr = Y.Color.fromArray([ r,
				g, b ], Y.Color.TYPES.RGB);
