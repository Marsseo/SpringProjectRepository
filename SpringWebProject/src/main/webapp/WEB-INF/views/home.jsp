					...
					.use(
							...
							function(Y) {
								...
								function updateColors() {
									r = rSlider.get('value'), g = gSlider
											.get('value'), b = bSlider
											.get('value'), rgbStr = Y.Color
											.fromArray([ r, g, b ],
													Y.Color.TYPES.RGB);
							...
							<div class="col-md-4">
								<div id="6ChartContainer" class="box-design2">
									<div class="budget-wrap">
										<div class="budget">
											<div class="header">
												<input type="hidden" id="hiddenleftright"
													value="${leftright}" /> <input type="hidden"
													id="hiddenupdown" value="${updown}" />
												<div class="title clearfix" style="display: inline-block:">
													<span>Camera Control </span> <span style="float: right;">좌우
														: <span class="pull-right"> </span>
													</span> <span style="float: right;">상하 : <span id="upDown"
														style="float: right;"> </span>
													</span> &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;
												</div>
											</div>
											<div class="content" style="display: inline;">
												<input id="rangeslider1" type="range" min="10" max="100"
													value="45" data-rangeslider> <br/>
												<br/> <input id="rangeslider0" type="range" min="10" max="170"
													value="90" data-rangeslider>
											</div>
										</div>
									</div>
								</div>
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div id="6c"
									...
									<input style="float: left; height: 220px; width: 180px;"
										id="laserOn"
										<c:if test="${laseremitterStatus=='off'}">onclick="laseremitter('change','on')" </c:if>
										<c:if test="${laseremitterStatus=='on'}">onclick="laseremitter('change','off')" </c:if>
										<c:if test="${laseremitterStatus=='off'}">type="image" src="<%=application.getContextPath()%>/resources/image/laser.png"</c:if>
										<c:if test="${laseremitterStatus=='on'}">type="image" src="<%=application.getContextPath()%>/resources/image/laserOn.PNG"</c:if> />
									<input style="float: right; height: 220px; width: 180px;"
										id="buzzerOn"
										<c:if test="${buzzerStatus=='off'}">onclick="buzzer('change','on')" </c:if>
										<c:if test="${buzzerStatus=='on'}">onclick="buzzer('change','off')" </c:if>
										<c:if test="${buzzerStatus=='off'}">type="image" src="<%=application.getContextPath()%>/resources/image/buzzer.png"</c:if>
										<c:if test="${buzzerStatus=='on'}">type="image" src="<%=application.getContextPath()%>/resources/image/buzzerOn.png"</c:if> />
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-md-4">
								<div id="1ChartContainer" class="box-design1" align="center">
									<div align="center" align="center"
										style="padding: 30px; background: url('/SpringWebProject/resources/image/display2.png'); background-repeat: no-repeat; background-position: center; text-align: center; height: 250px; width: 300px;">
										<div
											style="text-align: left; margin-left: 30px; font-size: 20px; font-style: italic; font-weight: bold;">LCD
											Display</div>
										<div style="text-align: left; margin-left: 30px;">
											<span id="lcdStatus"
												style="text-align: left; color: #3ADF00; font-weight: bold; font-size: 17px;">
												<br />▶ ${lcdline0} <br />▶ ${lcdline1}
											</span>
										</div>
										<br />
										<div
											style="text-align: left; height: 100px; padding: 20px; border: 10px solid blue; background-color: white;">
											<span id="lcdStatus" style="text-align: left;"> ▷ <input
												id="lcdline0" type="text" maxlength="16" value="Spring"
												style="color: black; width: 120px; border: 0px; border-bottom: 1px solid #2E64FE" />
												<br /> ▷ <input id="lcdline1" type="text" maxlength="16"
												value="Web Project"
												style="color: black; width: 120px; border: 0px; border-bottom: 1px solid #2E64FE" />
												<span style="font-size: 20px"> <span
													class="glyphicon glyphicon-ok" onclick="lcd('change')"></span>
											</span>
											</span>
										</div>
									</div>
								...
								<div id="2ChartContainer" class="box-design1">
									...
									<div style="float: left; background-color: white">
										<input type="hidden" id="angle" value="${fronttireAngle}" /> <input
											type="hidden" id="speed" value="${backtireSpeed}" /> <input
											type="hidden" id="direction" value="${backtireDirection}" />
										<button
											onclick="backtire('change', '${backtireDirection}','1000')"
											style="color: black;">1</button>
										<button
											onclick="backtire('change', '${backtireDirection}','1500')"
											style="color: black;">2</button>
										<button
											onclick="backtire('change', '${backtireDirection}','2200')"
											style="color: black;">3</button>
										<button
											onclick="backtire('change', '${backtireDirection}','2800')"
											style="color: black;">4</button>
										<button
											onclick="backtire('change', '${backtireDirection}','3500')"
											style="color: black;">5</button>
										<button
											onclick="backtire('change', '${backtireDirection}','4090')"
									...
									<button type="button" class="btn btn-warning"
										onclick="backtire('change', 'forward','${backtireSpeed}')"
										style="margin-bottom: 5px;">전진</button>
									<button type="button" class="btn btn-info"
										onclick="backtire('change', 'backward','${backtireSpeed}')"
										style="margin-bottom: 5px;">후진</button>
									<br />
									<button onmousedown="accelerator('${backtireDirection}')"
										onmouseup="decelerate('${backtireDirection}','${backtireSpeed}')"
										style="color: black;">										
										가속 패달
									</button>
							...
							<div class="col-md-4">
								<div id="container-speed" class="box-design1"></div>
							</div>
						</div>
						<div class="row">
								<div class="col-md-4" >
